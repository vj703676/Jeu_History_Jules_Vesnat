class Scene3_5 extends Phaser.Scene {
    ...
    create(){
      ...
      localStorage.setItem('lastScene', 'Scene3_5');
        ...
        this.niveau= this.add.text(104 , 857, 'Niveau\n  6/7', { fontSize: '25px', fill: '#000' }).setScale(1.1).setDepth(2);
        ...
        this.textevictoire= this.add.text(1380, 650, 'Rendez moi la monnaie\nsur 8 euros et 50 centimes\npour un rubis de Paris\nà 3 euros et 11 centimes', { fontSize: '25px', fill: '#000' }).setScale(1.25);
        ...
        this.count04= this.add.text(1150, 160, '3', { fontSize: '32px', fill: '#FFF' }).setScale(1.8);
    ...
    function limit5(number4){
      ...
      function limit6(number5){
        ...
        const clickButton8 = this.add.image(1190, 880, 'refresh').setScale(0.25).setInteractive().on('pointerdown', () => this.scene.start("Scene3_5")); 
    ...
    incrementClickCount6() {
      ...
      checkscore(){
        if((this.score == this.montant) && (this.score1 == this.montant1)){
            ...
            this.time.addEvent({ delay: 5000, callback: ()=>{ this.scene.start("Scene3_6") ;}, loop: false });
        }else {
